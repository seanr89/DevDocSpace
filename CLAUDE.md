...
**Portal-authored content (DB overlay).** `IContentStore` is registered as the scoped `OverlayContentStore`, which wraps the singleton `FileSystemContentStore`: `ApiSpec.Content` and `DocPage` rows are served in preference to files at the same key, so admins can create content in the portal or override ingested files and revert by clearing the DB copy. `ListSpecsAsync` (used by sync) stays filesystem-only; `ApiSpec.Path` is null for DB-only specs. Admin authoring endpoints live in `AdminContentEndpoints` (`POST /admin/specs`, `GET|PUT|DELETE /admin/specs/{service}/{version}/content`, `GET /admin/docs/{ns}/pages`, `PUT|DELETE /admin/docs/{ns}/pages/{*path}`) and report each item's `source` as `ingested`, `managed` (DB only) or `overridden`. Keys are validated by `ContentValidation` (lowercase slugs; mirrored client-side in `apps/web/src/lib/content-validation.ts`). The web editors are under `apps/web/src/app/(protected)/admin/{specs,docs}/`.
